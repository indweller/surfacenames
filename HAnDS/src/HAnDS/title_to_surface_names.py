...
def process_file(input_file_path, empty_dict):
    """
    ...
    for link in soup.find_all('a'):
        ...
        if link_title in empty_dict:
            if 'surface_names' not in empty_dict[link_title]:
                empty_dict[link_title]['surface_names'] = {}
            if text not in empty_dict[link_title]['surface_names']:
                empty_dict[link_title]['surface_names'][text] = 0
            empty_dict[link_title]['surface_names'][text] += 1
            found += 1
        else:
            key = link_title
            empty_dict[key] = {}
            if 'surface_names' not in empty_dict[key]:
                empty_dict[key]['surface_names'] = {}
            if text not in empty_dict[key]['surface_names']:
                empty_dict[key]['surface_names'][text] = 0
            empty_dict[key]['surface_names'][text] += 1
            found_redirect += 1
...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        process_file(file, empty_dict)
        file_count += 1
        print(file, file_count*100/file_size)
    # pickle dump
    pickle.dump(empty_dict, open(sys.argv[2], 'wb'))
